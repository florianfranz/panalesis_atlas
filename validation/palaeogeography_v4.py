...
_section   = config["validation"]["palaeogeography"]
...
for idx, row in stats_df.iterrows():
    ...
    print(f"\n  REFERENCE (ETOPO 2022):")
    print(f"    Mean: {row['Ref_mean']:.1f} ± {row['Ref_stderr']:.1f} m")
    print(f"    Median: {row['Ref_median']:.1f} m")
    print(f"    Std deviation: {row['Ref_std']:.1f} m")
    print(f"    Range: [{row['Ref_min']:.1f}, {row['Ref_max']:.1f}] m (span: {row['Ref_range']:.1f} m)")
    print(f"    Skewness: {row['Ref_skewness']:.3f}")
    print(f"    Kurtosis: {row['Ref_kurtosis']:.3f}")

    print(f"\n  PANALESIS MODEL:")
    print(f"    Mean: {row['Model_mean']:.1f} ± {row['Model_stderr']:.1f} m")
    print(f"    Median: {row['Model_median']:.1f} m")
    print(f"    Std deviation: {row['Model_std']:.1f} m")
    print(f"    Range: [{row['Model_min']:.1f}, {row['Model_max']:.1f}] m (span: {row['Model_range']:.1f} m)")
    print(f"    Skewness: {row['Model_skewness']:.3f}")
    print(f"    Kurtosis: {row['Model_kurtosis']:.3f}")

    print(f"\n  ERROR METRICS (Model - Reference):")
    print(f"    Mean bias: {row['Mean_Bias']:.1f} m ({row['Bias_Percent']:.2f}%)")
    print(f"    RMS difference: {row['RMS_Diff']:.1f} m  ← PRIMARY METRIC")
    print(f"    Mean absolute error: {row['MAE']:.1f} m")
    print(f"    Std of errors: {row['Std_Diff']:.1f} m")
    print(f"    Error range: [{row['Min_Error']:.1f}, {row['Max_Error']:.1f}] m")
    print(f"    Max absolute error: {row['Max_AbsDiff']:.1f} m")

    print(f"\n  ERROR DISTRIBUTION (Absolute):")
    print(f"    5th percentile: {row['P5_AbsDiff']:.1f} m")
    print(f"    10th percentile: {row['P10_AbsDiff']:.1f} m")
    print(f"    25th percentile: {row['P25_AbsDiff']:.1f} m")
    print(f"    50th percentile (median): {row['P50_AbsDiff']:.1f} m")
    print(f"    75th percentile: {row['P75_AbsDiff']:.1f} m")
    print(f"    90th percentile: {row['P90_AbsDiff']:.1f} m")
    print(f"    95th percentile: {row['P95_AbsDiff']:.1f} m")
    print(f"    99th percentile: {row['P99_AbsDiff']:.1f} m")
    ...
    if abs(rho_diff) > 0.05:
        if rho_diff > 0:
            print(f"    → Spearman > Pearson: Model captures topographic patterns")
    ...
    print(f"    Wasserstein distance: {row['Wasserstein_distance']:.1f} m")
...
sns.kdeplot(ref_vals_global, ax=ax1, color=COLOR_REF, linewidth=2, label='ETOPO (NOAA, 2022)')
sns.kdeplot(model_vals_global, ax=ax1, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANALESIS (This Study)')
...
if ANALYZE_CONTINENTAL_ONLY:
    ...
    sns.kdeplot(ref_vals_cont, ax=ax2, color=COLOR_REF, linewidth=2, label='ETOPO')
    sns.kdeplot(model_vals_cont, ax=ax2, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANALESIS')
...
if ANALYZE_OCEANIC_ONLY:
    ...
    sns.kdeplot(ref_vals_ocean, ax=ax3, color=COLOR_REF, linewidth=2, label='ETOPO')
    sns.kdeplot(model_vals_ocean, ax=ax3, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANALESIS')
...
ax4.set_xlabel('ETOPO Elevation [m]', fontsize=11)
ax4.set_ylabel('PANALESIS Elevation [m]', fontsize=11)
...
ax5.axvline(np.mean(errors_global), color='blue', linestyle='--', linewidth=2,
            label=f'Mean bias: {np.mean(errors_global):.1f} m')
ax5.set_xlabel('Error [m] (Model - Reference)', fontsize=11)
