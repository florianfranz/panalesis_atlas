...
_section   = config["validation"]["crustal_thickness"]
...
print(f"Model raster shape: {model_data.shape}")
...
# =============================================================================
print("\n" + "=" * 80)
print("CRUSTAL THICKNESS VALIDATION RESULTS")
print("=" * 80)
...
for idx, row in stats_df.iterrows():
    ...
    if abs(rho_diff) > 0.05:
        if rho_diff > 0:
            print(f"    → Spearman > Pearson: Model captures crustal thickness patterns")
...
sns.kdeplot(ref_vals_global, ax=ax1, color=COLOR_REF, linewidth=2, label='CRUST 2.0 (Bassin et al., 2000)')
sns.kdeplot(model_vals_global, ax=ax1, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANCRUST (This Study)')
ax1.set_xlabel('Crustal Thickness [km]', fontsize=11)
...
if ANALYZE_CONTINENTAL_ONLY:
    ...
    sns.kdeplot(ref_vals_cont, ax=ax2, color=COLOR_REF, linewidth=2, label='CRUST 2.0')
    sns.kdeplot(model_vals_cont, ax=ax2, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANCRUST')
    ax2.set_xlabel('Crustal Thickness [km]', fontsize=11)
...
if ANALYZE_OCEANIC_ONLY:
    ...
    sns.kdeplot(ref_vals_ocean, ax=ax3, color=COLOR_REF, linewidth=2, label='CRUST 2.0')
    sns.kdeplot(model_vals_ocean, ax=ax3, color=COLOR_MODEL, linewidth=2, linestyle='--', label='PANCRUST')
    ax3.set_xlabel('Crustal Thickness [km]', fontsize=11)
...
ax4.set_xlabel('CRUST 2.0 Thickness [km]', fontsize=11)
ax4.set_ylabel('PANCRUST Thickness [km]', fontsize=11)
