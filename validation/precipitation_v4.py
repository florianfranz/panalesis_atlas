...
_section   = config["validation"]["precipitation"]
...
for idx, row in stats_df.iterrows():
    ...
    print(f"    Mean: {row['Ref_mean']:.1f} ± {row['Ref_stderr']:.1f} mm/yr")
    print(f"    Median: {row['Ref_median']:.1f} mm/yr")
    print(f"    Std deviation: {row['Ref_std']:.1f} mm/yr")
    print(f"    Range: [{row['Ref_min']:.1f}, {row['Ref_max']:.1f}] m (span: {row['Ref_range']:.1f} mm/yr)")
    print(f"    Skewness: {row['Ref_skewness']:.3f}")
    print(f"    Kurtosis: {row['Ref_kurtosis']:.3f}")

    print(f"\n  PANALESIS MODEL:")
    print(f"    Mean: {row['Model_mean']:.1f} ± {row['Model_stderr']:.1f} mm/yr")
    print(f"    Median: {row['Model_median']:.1f} mm/yr")
    print(f"    Std deviation: {row['Model_std']:.1f} mm/yr")
    print(f"    Range: [{row['Model_min']:.1f}, {row['Model_max']:.1f}] m (span: {row['Model_range']:.1f} mm/yr)")
    print(f"    Skewness: {row['Model_skewness']:.3f}")
    print(f"    Kurtosis: {row['Model_kurtosis']:.3f}")

    print(f"\n  ERROR METRICS (Model - Reference):")
    print(f"    Mean bias: {row['Mean_Bias']:.1f} mm/yr ({row['Bias_Percent']:.2f}%)")
    print(f"    RMS difference: {row['RMS_Diff']:.1f} mm/yr  ← PRIMARY METRIC")
    print(f"    Mean absolute error: {row['MAE']:.1f} mm/yr")
    print(f"    Std of errors: {row['Std_Diff']:.1f} mm/yr")
    print(f"    Error range: [{row['Min_Error']:.1f}, {row['Max_Error']:.1f}] mm/yr")
    print(f"    Max absolute error: {row['Max_AbsDiff']:.1f} mm/yr")

    print(f"\n  ERROR DISTRIBUTION (Absolute):")
    print(f"    5th percentile: {row['P5_AbsDiff']:.1f} mm/yr")
    print(f"    10th percentile: {row['P10_AbsDiff']:.1f} mm/yr")
    print(f"    25th percentile: {row['P25_AbsDiff']:.1f} mm/yr")
    print(f"    50th percentile (median): {row['P50_AbsDiff']:.1f} mm/yr")
    print(f"    75th percentile: {row['P75_AbsDiff']:.1f} mm/yr")
    print(f"    90th percentile: {row['P90_AbsDiff']:.1f} mm/yr")
    print(f"    95th percentile: {row['P95_AbsDiff']:.1f} mm/yr")
    print(f"    99th percentile: {row['P99_AbsDiff']:.1f} mm/yr")
    ...
    if abs(rho_diff) > 0.05:
        if rho_diff > 0:
            print(f"    → Spearman > Pearson: Model captures precipitation patterns")
    ...
    print(f"    Wasserstein distance: {row['Wasserstein_distance']:.1f} mm/yr")
...
ax1.set_xlabel('Precipitation [mm/yr]', fontsize=11)
...
stats_text = f"KS: {global_row['KS_statistic']:.4f}\nWD: {global_row['Wasserstein_distance']:.0f} mm/yr"
...
if ANALYZE_CONTINENTAL_ONLY:
    ...
    ax2.set_xlabel('Precipitation [mm/yr]', fontsize=11)
    ...
    stats_text = f"KS: {cont_row['KS_statistic']:.4f}\nWD: {cont_row['Wasserstein_distance']:.0f} mm/yr"
...
if ANALYZE_OCEANIC_ONLY:
    ...
    ax3.set_xlabel('Precipitation [mm/yr]', fontsize=11)
    ...
    stats_text = f"KS: {ocean_row['KS_statistic']:.4f}\nWD: {ocean_row['Wasserstein_distance']:.0f} mm/yr"
...
ax4.set_xlabel('PLASIM (Holden et al, 2016) Precipitation [mm/yr]', fontsize=11)
ax4.set_ylabel('PLASIM with PANALESIS Precipitation [mm/yr]', fontsize=11)
...
ax5.axvline(np.mean(errors_global), color='blue', linestyle='--', linewidth=2,
            label=f'Mean bias: {np.mean(errors_global):.1f} mm/yr')
ax5.set_xlabel('Error [mm/yr] (Model - Reference)', fontsize=11)
...
ax6.set_ylabel('Error [mm/yr]', fontsize=11)
